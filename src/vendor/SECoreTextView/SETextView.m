...
{
    [self addObject:object size:size replaceRange:NSMakeRange(index, 0) customData:customData];
}

- (void)addObject:(id)object size:(CGSize)size replaceRange:(NSRange)range customData:(NSDictionary*)customData
{
    SETextAttachment *attachment = [[SETextAttachment alloc] initWithObject:object size:size range:range customData:customData];
...
- (void)setParagraphStyle
{
    ...
    CGFloat firstLineHeadIndent = roundf(self.firstLineHeadIndent);
    CGFloat lineHeightMultiple = roundf(self.lineHeightMultiple);
...
- (void)updateLayout
{
//    NSLog(@"textView.bounds:%@", NSStringFromRect(self.bounds));
    [self setAdditionalAttributes];
    
//    NSLog(@"textLayout.bounds1:%@", NSStringFromRect(self.textLayout.bounds));
    self.textLayout.bounds = self.bounds;
...
//    NSLog(@"textLayout.bounds2:%@", NSStringFromRect(self.textLayout.bounds));
}
...
- (void)__highlightLinks
{
    [self enumerateLinksUsingBlock:^(SELinkText *link, BOOL *stop) {
        for (SETextGeometry *geometry in link.geometries) {
            ...
            UIBezierPath *path = [self bezierPathWithRoundedRect:linkRect cornerRadius:3.0f];
            [path fill];
        }
    }];
}
...
{
//    NSLog(@"drawRect, self.bounds=%@", NSStringFromCGSize(self.bounds.size));
...
//粘贴消息响应方法
- (void)pasteMessage:(id)sender
{
    if ([self.delegate respondsToSelector:@selector(textView:customPasteMessage:)]) {
        [self.delegate textView:self customPasteMessage:sender];
    }
}
...
- (void)showEditingMenu
{
    ...
    NSArray* menuItems = [menuController menuItems];
    
    //加入自定义输入框弹出菜单
    if (menuItems.count==0) {
        UIMenuItem* menuItem = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Paste Message", nil) action:@selector(pasteMessage:)];
...
//    if (action == @selector(pasteCustomData:) && self.isEditing) {
//        return YES;
//    }
    ...
    //检测自定义子菜单
    if (action == @selector(pasteMessage:)) {
        return YES;
    }
//    if (self.additionalMenuItems.count>0) {
//        for (UIMenuItem* menuItem in self.additionalMenuItems) {
//            if (action == menuItem.action) {
////                if([self.delegate respondsToSelector:menuItem.action])
//                return YES;
//            }
//        }
//    }
...
//{
////加上这段代码后出现交叉调用，导致不能正常弹出键盘 (IOS8下编译出现，IOS7下编译没问题)
...
- (void)paste:(id)sender
{
    ...
    //回调外部函数执行粘贴操作，并返回是否执行下一步操作
    if ([self.delegate respondsToSelector:@selector(textView:paste:)]) {
        needContinue = [self.delegate textView:self paste:sender];
    }
...
- (void)clearAll
{
//    self.textLayout.textSelection = nil; //取消选中内容
    [self.textLayout selectAll]; //选中全部内容
...
//    NSLog(@"insertText:%@", NSStringFromCGSize(self.frame.size));
...
- (void)insertObject:(id)object size:(CGSize)size customData:(NSDictionary*)customData
{
    ...
    attachment = [[SETextAttachment alloc] initWithObject:object size:size range:NSMakeRange(location, replacementCharacter.length) customData:customData];
    [self.attachments addObject:attachment];
